...
public class ExplorationAbilityHandler {
    ...
    public static void onBreakSpeed(PlayerEvent.BreakSpeed event) {
        ...
        AdventureProgressCapability.getAdventureProgress(player).ifPresent(progress -> {
            ...
            if (progress.isFullyUnlocked()) {
                multiplier *= com.rosycentury.adventure_power.config.ModConfig.AWAKEN_MULTIPLIER.get().floatValue();
            }
    ...
    private static void syncReachAttribute(Player player, boolean enabled, int milestones, boolean fullyUnlocked) {
        ...
        if (enabled) {
            ...
            if (fullyUnlocked) {
                bonus *= com.rosycentury.adventure_power.config.ModConfig.AWAKEN_MULTIPLIER.get().floatValue();
            }
    ...
    private static void syncVitalityAttribute(Player player, boolean enabled, int milestones, boolean fullyUnlocked) {
        ...
        if (enabled) {
            ...
            float bonus = ability.value(milestones);
            if (fullyUnlocked) {
                bonus = (float) Math.ceil(bonus * com.rosycentury.adventure_power.config.ModConfig.AWAKEN_MULTIPLIER.get());
            }
