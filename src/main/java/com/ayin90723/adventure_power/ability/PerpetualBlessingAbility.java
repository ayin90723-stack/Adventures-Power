 ...
 * 恩赐永驻 — 药水效果持续时间延长。
 * 解锁条件：1 里程碑
 * 成长公式：base + step × ((milestones - required) / 2)（每 2 里程碑一跳）
...
public class PerpetualBlessingAbility implements Ability {
    ...
    /**
     * 返回延长的 tick 数。从配置读取 base + step × ((milestones - required) / 2)。
     */
    @Override
    public float value(int milestones) {
        return ModConfig.PERPETUAL_BLESSING_BASE.get() + ModConfig.PERPETUAL_BLESSING_STEP.get() * ((milestones - requiredMilestones()) / 2);
    }
}
